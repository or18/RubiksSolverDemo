...
enum class BucketModel {
    ...
    // XCross+FreePair models (optimized for 17 goal states)
    WASM_XCROSS_MOBILE_STD,   // 1M/1M/2M/2M (~332 MB single, ~664 MB dual-heap) - NEW DEFAULT
    WASM_XCROSS_MOBILE_LOW,   // 1M/1M/1M/1M (~332 MB single, ~664 MB dual-heap) - Minimum
    WASM_XCROSS_MOBILE_HIGH,  // 1M/1M/2M/4M (~398 MB single, ~796 MB dual-heap) - High-end mobile
    
    // XXCross models (legacy, for reference)
...
struct ModelData {
    size_t d6, d7, d8;           // Bucket sizes (bytes)
    size_t measured_rss_mb;      // Peak RSS in MB (no margin)
    size_t total_nodes_million;  // Total nodes explored (debugging)
    double load_d6, load_d7, load_d8;  // Load factors (debugging)
    
    ModelData() : d6(0), d7(0), d8(0), measured_rss_mb(0), 
                  total_nodes_million(0), load_d6(0), load_d7(0), load_d8(0) {}
    
    ModelData(size_t bucket7, size_t bucket8, size_t bucket9,
              size_t rss, size_t nodes, double ld6, double ld7, double ld8)
        : d6(bucket7), d7(bucket8), d8(bucket9)
        , measured_rss_mb(rss), total_nodes_million(nodes)
        , load_d6(ld6), load_d7(ld7), load_d8(ld8) {}
};
...
struct BucketConfig {
    ...
    size_t custom_bucket_6 = 0;  // Must be power of 2 (1M-64M)
    size_t custom_bucket_7 = 0;
    size_t custom_bucket_8 = 0;
    size_t custom_bucket_9 = 0; // Depth 9 bucket (NEW: for volume peak coverage)
...
struct ResearchConfig {
    ...
    int force_full_bfs_to_depth = -1;         // -1=auto, 6-9=force BFS to this depth
...
inline const std::unordered_map<BucketModel, ModelData>& get_measured_data() {
    static const std::unordered_map<BucketModel, ModelData> MEASURED_DATA = {
        ...
        // Format: d6/d7/d8/d9, single_heap_mb, dual_heap_mb (2x), total_nodes
...
inline void validate_custom_buckets(const BucketConfig& cfg, const ResearchConfig& research) {
    ...
    if (!is_power_of_two(cfg.custom_bucket_6) ||
        !is_power_of_two(cfg.custom_bucket_7) ||
    ...
    if (cfg.custom_bucket_6 < MIN_BUCKET || cfg.custom_bucket_6 > MAX_BUCKET ||
        cfg.custom_bucket_7 < MIN_BUCKET || cfg.custom_bucket_7 > MAX_BUCKET ||
    ...
    // Check 4: If bucket_d9 specified, validate it too
    if (cfg.custom_bucket_9 > 0) {
        if (!is_power_of_two(cfg.custom_bucket_9)) {
            throw std::runtime_error(
                "Custom bucket_d9 must be power of 2 (1M, 2M, 4M, 8M, 16M, 32M, 64M)"
            );
        }
        if (cfg.custom_bucket_9 < MIN_BUCKET || cfg.custom_bucket_9 > MAX_BUCKET) {
            throw std::runtime_error(
                "Custom bucket_d9 must be in range [1M, 64M]"
            );
        }
    }
}

inline size_t estimate_custom_rss(size_t d6_bucket, size_t d7_bucket, size_t d8_bucket) {
    // Conservative estimates based on empirical load factors
    const double LOAD_d6 = 0.90;  // depth 7 typical load
    const double LOAD_d7 = 0.89;  // depth 8 typical load  
    const double LOAD_d8 = 0.70;  // depth 9 typical load
    const size_t BASE_OVERHEAD_MB = 50;  // Solver overhead, vectors, etc.
    
    size_t d6_rss = (d6_bucket >> 20) * LOAD_d6;  // Convert to MB
    size_t d7_rss = (d7_bucket >> 20) * LOAD_d7;
    size_t d8_rss = (d8_bucket >> 20) * LOAD_d8;
    
    return d6_rss + d7_rss + d8_rss + BASE_OVERHEAD_MB;
}
...
inline void validate_model_safety(BucketModel model, size_t budget_mb, 
    ...
    if (model == BucketModel::CUSTOM && !research.ignore_memory_limits) {
        size_t estimated_rss_mb = estimate_custom_rss(cfg.custom_bucket_6, 
                                                       cfg.custom_bucket_7, 
                                                       cfg.custom_bucket_8);
