...
// Web Worker for xcross_free_pair solver database construction and scramble generation

console.log('[Worker] Script starting...');

let isInitialized = false;
let solver = null;  // Persistent xcross_free_pair_search instance for random scramble generation
let buildLogs = [];  // Collect logs during database build for statistics

// Initialize WASM module (non-MODULARIZE build)
console.log('[Worker] Creating init promise...');
const initPromise = new Promise((resolve, reject) => {
    console.log('[Worker] Inside init promise');
    self.Module = {
        onRuntimeInitialized: () => {
            // Don't create solver instance here - will be created during build
            console.log('[Worker] WASM Module initialized - sending initialized message');
            isInitialized = true;
            self.postMessage({ type: 'initialized' });
            console.log('[Worker] Initialized message sent');
            resolve();
        },
        print: function(text) {
...
console.log('[Worker] About to load test_solver.js...');
importScripts('test_solver.js');
console.log('[Worker] test_solver.js loaded');
